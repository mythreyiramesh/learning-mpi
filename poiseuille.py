...
while (global_error[0]>tolerance):
    ...
    U_local_new[1:-1,1:-1] = U_local_old[1:-1,1:-1]+constant+prefactor*(U_local_old[:-2,1:-1]+U_local_old[2:,1:-1]-2*U_local_old[1:-1,1:-1])
    print(np.amax(np.abs(U_local_new-U_local_old)),rank)
    local_error[0] = np.sum(np.power(np.abs(U_local_new[1:-1,1:-1]-U_local_old[1:-1,1:-1]),2))
